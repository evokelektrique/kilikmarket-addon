...
window.km_get_product = function() {
	...
	if(document.querySelectorAll(".stock-out").length <= 0) {
		...
		// Huawei doens't have static weight for products
		product.weight_to_kilogram = null;

		// Get First image
		product.image = document.querySelector(".pro-gallery-area a:first-child").href || null;

		// Product Title
		var product_title = document.getElementById('pro-name').textContent || "NO TITLE";

		// Product Description
		product.description = "No description";

		// Product Categories
		product.categories = ["Huawei"];

		// Product Price
		var product_price = document.getElementById('pro-special-price').textContent.split("AED")[1].trim().replace(',','');
		...
		var selected_values = document.querySelectorAll('.skuPart dl dd li.selected');
		document.querySelectorAll(".skuPart dl dt").forEach((title, index) => {
			options.push(title.textContent.replace(' ', ''), selected_values[index].textContent.trim());
		})
...
function kilikmartket_main() {
	...
	var clear_notices = setInterval(() => {
		if(document.getElementById("cookie_notices") != null) {
			document.getElementById("cookie_notices").style.display = "none";
			clearInterval(clear_notices);
		}
	}, 1000);
