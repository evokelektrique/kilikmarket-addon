...
window.km_get_product = function() {
	...
	if(document.querySelectorAll(".page-productDetails").length > 0) {
		...
		// Virgin doens't have static weight for products
		product.weight_to_kilogram = null;

		// Get First image
		product.image = document.querySelector('.pdp_image-carousel-image ').style.background.replace('url("', "").replace('")', "") || null;

		// Product Title
		var product_title = document.querySelector('h1.name').textContent.trim() || "NO TITLE";

		// Product Description
		product.description = "No description";

		// Product Categories
		product.categories = ["Virgin"];

		// Product Price
		var product_price = document.querySelector('.price__number').textContent.trim().replace(",", "");
