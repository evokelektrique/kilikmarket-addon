    ...
    return function (method, url, async) {
        ...
        if (!url.startsWith(server_address + '')) {
            if (url.startsWith('http')) {
                ...
                url = correctUrl[1];
                url = server_address + "&url=https://www.amazon.ae" + url;
                this.withCredentials = true;
            } else {
                let proxyServerAddress = server_address + "&url=https://www.amazon.ae";
                if (!url.startsWith('/')) {
...
function kilikmartket_main() {
	...
	document.addEventListener('DOMContentLoaded', function(event) {
		...
		document.getElementById('nav-search-submit-text').addEventListener('click', () => {
			...
			var search_url = `${server_address}&url=https://www.amazon.ae/s?k=${search_text}&i=${category}`;
